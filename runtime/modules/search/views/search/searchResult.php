...
if($SearchKey) {
    $listings = Listings::homeSearch($SearchKey, $searchCriteria, $searchViewOffset, $searchViewLimit, $searchOrderBy);
}else{
    $listings = array();
}
$totalComments = sizeof($listings);
...
<script type="text/javascript">
    ...
    $("#totalComment").html("("+<?=Listings::getTotalComments()?>+")");

</script>


    <div class="searchPage">
    <h1 style="font-size: 2em;">Results for <span style="color:#e5a04d;"><?php echo $searchKey; ?></span></h1>
    <h5 style="color:#A84793; font-size: 1em;">You searched for <span style="color:#e5a04d;"><?php echo $searchKey; ?></span> -
        <?php echo $totalComments;?> results found</h5>
    <div>&nbsp;</div>
        <div class="sortby">
    Sort by:&nbsp;&nbsp;
    Most Recent <input type="radio" name="searchByCriteria" id="searchByRecent"  class="searchByCriteria" value="drg_approvedate DESC">
    Oldest       <input type="radio" name="searchByCriteria" id="searchOldest"  class="searchByCriteria"  value="drg_approvedate ASC">
        ...
        if($listings) {
            foreach ($listings as $index => $listingData):
                ...
                <tr class="<?php echo $class; ?>" style="width:20%">
                    <td><?php
                        $user = new User();
                        $userData = User::model()->findByAttributes(array('drg_uid'=>$listingData['drg_uid']));

                        $user_dirname = strtolower($userData['drg_username'])."_".$userData['drg_id'];
                        $img_path = Yii::app()->getBaseUrl(true).'/upload/users/'.$user_dirname.'/listing/thumb/'.$listingData['drg_logo'];
                        ?>
                        <img src="<?php echo $img_path; ?>" class="searchImage" alt="Logo" /> </td>
                    <td style="width:80%;valign:top"><div class="listing_title"><?php echo $listingData['drg_title'] ?></div>
                     <?php   echo '<span class="listing_desc">'.$listingData['drg_desc'].'</span>';
                        echo '<br/><br/>';
                        echo $listingData['drg_explanation'];?>
                    </td>
                </tr>
...
?>
        <div style="background-color: #FFFFFF; padding: 15px; border: 1px solid #a1919c; border-radius: 10px;">
            <div style="margin:303px 16px 0 0;" class="close_caform"><a title="Close"
                                                                        href="<?php echo Yii::app()->baseUrl;?>" class="button white smallrounded">X</a></div>
  <?php echo '<div style="font-size: small;">No Search criteria to get the result</div>';} ?>
            </div>
